...
class Attention(torch.nn.Module):
    ...
    def forward(self, x, position_embeddings, past_key_value=None, use_cache=False, attn_mask=None):
        ...
        if self.flash and (seq_len > 1) and (not self.is_causal or past_key_value is None) and (attn_mask is None or torch.all(attn_mask == 1)):
            ...
            attn_output = self.attn_dropout(F.softmax(attn_scores.float(), dim=-1)).type_as(xq) @ xv
        attn_output = attn_output.transpose(1, 2).reshape(batch_size, seq_len, -1)
        attn_output = self.residual_dropout(self.out_proj(attn_output))
        return attn_output, past_kv

class FeedForward(nn.Module):
    def __init__(self, config: ChaokaiMindConfig, intermediate_size: int = None):
        super().__init__()
